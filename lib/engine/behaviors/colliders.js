 ...
 *
 * 当たり判定に関わる全ての素子はリジッドを持っている必要があるが、コライダはケースバイケース。
 * 例えば10体の素子がいてそれぞれがお互い全てに対して判定されるなら、すべての素子がコライダを持っている必要があるし、
...
class ColliderBehavior extends Behavior {
    ...
    stay(scene) {
        ...
        // 自分のリジッドを取得。
        var me = this.host.behaviors.need("rigid", RectRigid);

        // 相手の素子と一つずつ、判定していく。
        for( let competitor of Array.cast(this.target) ) {

            var he = competitor.behaviors.need("rigid", RectRigid);
...
class CircleRigid extends RigidBehavior {
    ...
    againstCircle(against) {
        ...
        // 自身と相手との距離を取得。
        var distance = this.rigid.center.sub(against.rigid.center).distance;
